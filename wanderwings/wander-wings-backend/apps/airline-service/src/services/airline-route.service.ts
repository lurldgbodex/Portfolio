...
export class AirlineRouteService {
    ...
    async addRouteToAirline(airlineId: number, addRouteData: AddRouteData): Promise<RouteResponse> {
        ...
        if (addRouteData.airline_price.length !== 2) {
            throw new BadRequestException('provide price data for economic and business class')
        }

        for (const pricePackage of addRouteData.airline_price) {
            if (!pricePackage.price || !pricePackage.customer_type) {
                throw new BadRequestException('airline_price must have price and customer_type')
            }

            if (!Object.values(CustomerType).includes(pricePackage.customer_type)) {
                throw new BadRequestException('customer_type must be either economic or business');
            }
        }
    ...
    async updateroute(airlineId: number, routeId: number, updateData: UpdateRouteData): Promise<RouteResponse> {
        ...
        if (!updateData || Object.keys(updateData).length === 0) {
            throw new BadRequestException('you need to provide at least one field to update');
        }
            ...
            if (updateData.price_package && updateData.price_package.length > 0) {
               for (const priceData of updateData.price_package) {
                const existingAirlinePrice = existingRoute.airlinePrice.find(
                    (price) => price.customerType.toLocaleLowerCase() === priceData.customer_type.toLocaleLowerCase()
                );
                ...
                if (existingAirlinePrice) {
                    ...
                    throw new BadRequestException("customer_type should be either economic or business");
                }                
               };
            }
    ...
    async deleteRoute(airlineId: number, routeId: number): Promise<{ message: string }> {
        ...
        return this.entityManager.transaction(
            async (manager: EntityManager) => {
                ...
                return { message: `Route deleted Successfully with id ${route}`};
            }
        )
    }
}