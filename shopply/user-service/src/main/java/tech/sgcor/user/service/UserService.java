...
public class UserService {
    ...
    public CreateUserResponse registerUser(CreateUserRequest request) {
        if (userRepository.findByEmail(request.getEmail()).isPresent()) {
            throw new UserExistsException("user already exists for this email");
        }
        ...
        // todo: call notification service to send registration notification to user

        return new CreateUserResponse(location, new CustomResponse(201, "user created successfully"));
    }
    ...
    public CustomResponse updateUserDetails(UpdateUserDetails request) {
        ...
        return new CustomResponse(200, "user details updated successfully");
    }
    ...
    public CustomResponse requestPasswordReset(String email) {
        ...
        // todo: send a email to user with token to reset password
